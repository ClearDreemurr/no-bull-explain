...
4. **Recreate the inventor's pain** — show why the naive approach breaks down, then introduce the mechanism as a solution that naturally emerged from a real problem.
5. **Choose an explanation path** — combine approaches based on where the user is stuck.
6. **Local before global** — make one cell, one state, or one object move first, then scale up to the full system.
7. **Map back to real concepts** — only map the essential structure; don't force every detail to correspond.
8. **Explain the simplification boundary** — where does this analogy, model, or derivation break down?
9. **Give a demystified definition** — ground it in data, state, rules, parameters, flow, permissions, cache, or mapping.

## Choosing an Explanation Path

- **Everyday analogy** — for motivation, roles, trust, collaboration, permissions, delegation, scheduling, incentives, resource allocation. The analogy must be structurally isomorphic — surface similarity is not enough.
- **Step-by-step reasoning or visual description** — for data flow, state changes, spatial relationships, math structures, system architecture, algorithms, circuits, network protocols.
- **Information restructuring** — for too many terms, branching complexity, model families, product comparisons, ecosystem history, terminological confusion.
- **Formula decomposition** — for loss functions, gradients, probability, optimization, scoring, encoding, numeric formats. Only show the formula after the intuitive scene and variable meanings are established.

## Anti-Jargon Rules

- Don't start with implementation symbols. Variable names, function names, class names, config keys, file paths, module names, formulas, and subscripts come later. Unless the user explicitly asks, don't open with `dp[i][j]`, `FooService.handle()`, or `config.xxx`.
...
- Light demystification is welcome ("this optimization saves space but creates a coverage headache"), but don't mimic a fixed persona or catchphrase.
...
- **Local focus** — in the first 3 frames, avoid showing the full table. Track one cell, one token, one window, one interval, one state, or one packet. Once the local mechanism is clear, generalize to the full matrix, sequence, protocol, or system.
- **Spatial externalization** — when the user needs to remember positions, ranges, or relationships, use text structures (rows, columns, cells, slots, windows, arrows, strikeouts, brackets) to move the mental space onto the page. Don't just say "the range got smaller" — let the user see which segment was kept and which was dropped.
...
- **Input-output closure** — when explaining recursion, generation, loops, RNNs, Transformers, or agents, explicitly write "the output of this step becomes the input of the next step." Closure prevents the user from seeing isolated steps without understanding why the process continues.

Applicable to: matrices/tensors, convolution, attention mechanisms, DP tables, sliding windows, network protocols, state machines, encoding formats, recursion, generation processes.

Avoid sacrificing accuracy for visual flair; avoid tracking multiple moving objects at once; avoid anthropomorphism in place of real mechanisms; don't over-visualize when the user needs rigorous definitions or proofs.

## Rules & Prohibitions

- Start from the minimal mechanism, not the full architecture diagram.
- Logic first, then name the term. A term should be the name of a solved problem.
...
This example demonstrates the explanation structure — learn from its delayed terminology, low-config scene, local dynamic reasoning, action-oriented description, delayed naming, and pause placement. The pauses are part of the example; do not deliver the full walkthrough in one shot.
...
But this needs a proper protocol, not just shouting. So your deskmate decides: every time they reply, they include a value called the window size. They tell you they can handle 5 notes. How do you use "5" to do flow control?

**【Pause 1】** Don't rush ahead. First decide: if they said they can receive up to 5 notes, should the 6th note be sent or not?

(Stop and wait for user reply)

### Minimal Model: Your Sending Slide Rail

You have an ordered stack of notes. Your deskmate's "5" is like a sliding frame that can only hold 5 notes.
...
You still have notes #6, #7, #8 ready, but you can't send them. This physical boundary is **flow control**.
...
**【Pause 3】** Core mechanism explained. Got it? If yes, next up: dynamic window resizing.

(Stop and wait for user reply)

### Dynamic Resizing

Above assumes the window size is fixed. But every time your deskmate replies with an acknowledgment, they can set a new window size. If they send 6, the frame expands by one. If they send 4, the frame shrinks — now only 4 slots fit.

This thing — with a size that limits the sender's transmission rate, sliding forward or resizing based on the receiver's state — is called: **sliding window**.

**Demystified definition:** Sliding window = a physical limit frame on the sender's side that slides forward and resizes itself based on the receiver's state, so the receiver never gets overwhelmed.
